...
-- Globals, mappack format, tile/entity encoding, and COLORSPACE are unchanged.
...
function level_filename(world, level, sublevel)
	if not sublevel or sublevel == 0 then
		return world .. "-" .. level
	end
	return world .. "-" .. level .. "_" .. sublevel
end

function level_detect_format_version(s)
	return string.match(s, BLOCKDELIMITER[1]) and 1 or 2
end

function level_delimiters(v)
...
function level_split_file_contents(s)
...
function level_count_tile_entries(t, multipledelimiter)
	local entries = 0
	for i = 1, #t do
		local parts = t[i]:split(multipledelimiter)
		if parts[2] then
			entries = entries + tonumber(parts[2])
		else
			entries = entries + 1
		end
	end
...
function level_decode_tile_tokens(t, mapwidth, mapheight, layerdelimiter, multipledelimiter)
	local map = {}
	local coinmap = {}
	for x = 1, mapwidth do
		map[x] = {}
	...
	local x, y = 1, 1
	for i = 1, #t do
		if string.find(t[i], multipledelimiter) then
...
function level_serialize_maptiles(map, coinmap, mapwidth, mapheight, v)
	local s = ""
	local mul = 1
	local prev = nil
...
function level_parse_tiles_only(s)
	local v, mapheight, t, s2, blockdelimiter, layerdelimiter, categorydelimiter, multipledelimiter, equalsign = level_split_file_contents(s)
	...
	local map, coinmap = level_decode_tile_tokens(t, mapwidth, mapheight, layerdelimiter, multipledelimiter)
	return {
		v = v,
		mapwidth = mapwidth,
		mapheight = mapheight,
...
function loadmap(filename, createobjects)
	...
	local v, parsed_height, t, s2, blockdelimiter, layerdelimiter, categorydelimiter, multipledelimiter, equalsign = level_split_file_contents(s)
