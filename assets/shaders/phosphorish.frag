...
vec4 effect(vec4 vcolor, Image tex, vec2 texture_coords, vec2 pixel_coords)
{
	...
	vec3 color = Texel(tex, texture_coords - 0.0 * one_x).rgb;
	vec3 color_prev = Texel(tex, texture_coords - 1.0 * one_x).rgb;
	vec3 color_prev_prev = Texel(tex, texture_coords - 2.0 * one_x).rgb;
