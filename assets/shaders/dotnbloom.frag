...
vec3 lookup(Image tex, float offset_x, float offset_y, vec2 coord)
{
...
vec4 effect(vec4 vcolor, Image tex, vec2 texcoord, vec2 pixel_coords)
{
	...
	// number a = Texel(tex, texcoord).a;

	vec2 c00 = texcoord + vec2(-dx, -dy);
	vec2 c10 = texcoord + vec2(  0, -dy);
	vec2 c20 = texcoord + vec2( dx, -dy);
	vec2 c01 = texcoord + vec2(-dx,   0);
	vec2 c11 = texcoord + vec2(  0,   0);
	vec2 c21 = texcoord + vec2( dx,   0);
	vec2 c02 = texcoord + vec2(-dx,  dy);
	vec2 c12 = texcoord + vec2(  0,  dy);
	vec2 c22 = texcoord + vec2( dx,  dy);

	vec3 mid_color = lookup(tex, 0.0, 0.0, c11);
	vec3 color = vec3(0.0);
	color += lookup(tex, -1.0, -1.0, c00);
	color += lookup(tex,  0.0, -1.0, c10);
	color += lookup(tex,  1.0, -1.0, c20);
	color += lookup(tex, -1.0,  0.0, c01);
	color += mid_color;
	color += lookup(tex,  1.0,  0.0, c21);
	color += lookup(tex, -1.0,  1.0, c02);
	color += lookup(tex,  0.0,  1.0, c12);
	color += lookup(tex,  1.0,  1.0, c22);
