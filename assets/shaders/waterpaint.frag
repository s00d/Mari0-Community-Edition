...
vec4 effect(vec4 vcolor, Image tex, vec2 texcoord, vec2 pixel_coords)
{
	...
	vec3 c00 = Texel(tex, texcoord - dg1).xyz;
	vec3 c01 = Texel(tex, texcoord - dx).xyz;
	vec3 c02 = Texel(tex, texcoord + dg2).xyz;
	vec3 c10 = Texel(tex, texcoord - dy).xyz;
	vec3 c11 = Texel(tex, texcoord).xyz;
	vec3 c12 = Texel(tex, texcoord + dy).xyz;
	vec3 c20 = Texel(tex, texcoord - dg2).xyz;
	vec3 c21 = Texel(tex, texcoord + dx).xyz;
	vec3 c22 = Texel(tex, texcoord + dg1).xyz;

	vec2 texsize = textureSize;
	
	vec3 first = mix(c00, c20, fract(texcoord.x * texsize.x + 0.5));
	vec3 second = mix(c02, c22, fract(texcoord.x * texsize.x + 0.5));
	
	vec3 mid_horiz = mix(c01, c21, fract(texcoord.x * texsize.x + 0.5));
	vec3 mid_vert = mix(c10, c12, fract(texcoord.y * texsize.y + 0.5));
	
	vec3 res = mix(first, second, fract(texcoord.y * texsize.y + 0.5));
